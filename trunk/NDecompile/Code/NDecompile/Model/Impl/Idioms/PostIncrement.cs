...
{
	internal class PostIncrement : Idiom
	{
		// ldarg
		// dup
		// ldc.i1
		...
		public override void Process(IList<IInstruction> instructions, int index)
		{
			...
			if ((instruction1.OpCode == OpCodes.Ldarg) &&
				...
				(instruction3.OpCode == OpCodes.Add) &&
				(instruction4.OpCode == OpCodes.Starg) &&
				((ushort)instruction4.Argument == (ushort)instruction1.Argument))
			{
				instructions[index + 1] = new Instruction(OpCodeTable.PostIncrementStatementOpCode, instruction1.IP, (int)instruction2.Argument);
			...
			if ((instruction1.OpCode == OpCodes.Ldarg) &&
				...
				(instruction4.OpCode == OpCodes.Add) &&
				(instruction5.OpCode == OpCodes.Starg) &&
				((ushort)instruction5.Argument == (ushort)instruction1.Argument))
			{
				instructions[index + 1] = new Instruction(OpCodeTable.PostIncrementOpCode, instruction1.IP, (int)instruction3.Argument);
				instructions.Remove(instruction3);
				instructions.Remove(instruction4);
			...
			if ((instruction1.OpCode == OpCodes.Ldarg) &&
				...
				((ushort)instruction5.Argument == (ushort)instruction1.Argument))
			{
				instructions[index + 1] = new Instruction(OpCodeTable.PostIncrementOpCode, instruction1.IP, (int)instruction2.Argument);
				instructions.Remove(instruction3);
				instructions.Remove(instruction4);
				instructions.Remove(instruction5);
			}
